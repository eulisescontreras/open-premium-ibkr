...
- `sys2/backtest/greeks.py` — Black-Scholes-Merton (math.erf, sin scipy) SOLO backtest. `parse_occ` (ticker OCC→expiry/right/strike), `t_years` (zoneinfo ET, no offset fijo §2.3), `bs_price`, `implied_vol` (bisección, suelo intrínseco §2.3), `greeks`, `desde_precio`. **Invierte el precio REAL de massive→IV→greeks (preserva la sonrisa); NUNCA fija precios** (§57: BS-IV-plana=60% error). **cr_greeks_bs VERDE**: round-trip 1e-6, delta analítica=fin-dif 9e-6, 0 fuera de rango, 0 paridad (invariante a misma sigma `|dC|+|dP|=e^{-qT}`), delta comprables p5 0.520/mediana 0.722/p95 0.950 ≈ H3. r=0.045 q=0.013 (constantes backtest; irán a config.py). Insight: las diferencias de IV call/put del mismo strike SON la sonrisa (skew), no un error.
- Cold runs verdes: `cr_schema.py`, `cr_migracion.py`, `cr_supertrend.py`, `cr_entradas.py`, `cr_greeks_bs.py`.
