...
BMOT, BPIP = MOT + ".ej.bak", PIP + ".ej.bak"
...
_TABS = float(_os.environ.get("RL_TABS", "0"))      # suelo ABSOLUTO minimo (0=off, usa TMIN fijo)
_DESC = float(_os.environ.get("RL_DESC", "0"))      # descuento al CERRAR (spread bid-ask real)
...
def _cfg_dia(_saldo, _nivmax, _racha=0):
    ...
    if _SIZE in ("frac", "fracmin"):
        ...
        # SUELO DINAMICO: en vez de PARAR cuando el saldo no cubre el suelo, se baja el suelo
        # hasta el minimo real del instrumento (_TABS ~ el vertical mas barato). Solo se para
        # cuando ni siquiera eso cabe. Sin esto la cuenta queda VIVA PERO CONGELADA.
        _suelo = _TMIN
        if _TABS:
            _suelo = max(_TABS, min(_TMIN, _saldo / max(_KSUP, 1.0)))
        if _KSUP and _saldo < _KSUP * _suelo:
            return None, _nivmax
        _t = max(_saldo * _FRAC * _fr, _suelo)
...
A_G = "                    g = ((pos['mid'] - pos['ask']) * 100 - C.COMISION) * pos.get('nq', 1)"
N_G = "                    g = ((pos['mid'] * (1 - _DESC) - pos['ask']) * 100 - C.COMISION) * pos.get('nq', 1)"
...
# SUELO DINAMICO vs SUELO FIJO. Con suelo fijo 140 la cuenta se CONGELA en ~460$ (4 de 7
# arranques quedan vivos pero sin operar). Con suelo dinamico se opera mas pequeño (hasta 88$,
# el vertical mas barato) en vez de parar. Se comparan los MISMOS 7 arranques.
# ¿ES EJECUTABLE EL OBJETIVO DEL 95%? El motor cierra al MID del vertical. En 0DTE cerca del
# maximo el spread bid-ask se ensancha y puede no haber comprador a ese precio. Se aplica un
# DESCUENTO creciente a TODOS los cierres (0% / 2% / 5% / 10%) para medir la sensibilidad.
# Unica evidencia real disponible (op131, 2026-08-18): lleno a 1.52 con ask 1.54 y 0.51 con
# bid 0.52 -> DENTRO del spread, mejor que el ask.
V = [
    ("e_d0_2408", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2024-08-15", "3.5", "0", "0", "0", "0.95", "0", "0"),
    ("e_d0_0218", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2025-02-18", "3.5", "0", "0", "0", "0.95", "0", "0"),
    ("e_d0_1015", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2025-10-15", "3.5", "0", "0", "0", "0.95", "0", "0"),
    ("e_d002_2408", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2024-08-15", "3.5", "0", "0", "0", "0.95", "0", "0.02"),
    ("e_d002_0218", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2025-02-18", "3.5", "0", "0", "0", "0.95", "0", "0.02"),
    ("e_d002_1015", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2025-10-15", "3.5", "0", "0", "0", "0.95", "0", "0.02"),
    ("e_d005_2408", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2024-08-15", "3.5", "0", "0", "0", "0.95", "0", "0.05"),
    ("e_d005_0218", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2025-02-18", "3.5", "0", "0", "0", "0.95", "0", "0.05"),
    ("e_d005_1015", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2025-10-15", "3.5", "0", "0", "0", "0.95", "0", "0.05"),
    ("e_d010_2408", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2024-08-15", "3.5", "0", "0", "0", "0.95", "0", "0.10"),
    ("e_d010_0218", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2025-02-18", "3.5", "0", "0", "0", "0.95", "0", "0.10"),
    ("e_d010_1015", "600", "0.18", "140", "3", "0.15", "4", "0", "0", "2", "0.195", "0.150", "0.031", "2025-10-15", "3.5", "0", "0", "0", "0.95", "0", "0.10"),
]
...
for txt, pat in ((bmot, A_IMP), (bmot, A_D), (bmot, A_SEN), (bmot, A_NQ), (bmot, A_NQ2),
                 ...
                 (bmot, A_FIN), (bmot, A_STOP), (bmot, A_SAL), (bmot, A_G), (bpip, P_IMP), (bpip, P_VIEJO)):
...
tm = (bmot.replace(A_IMP, N_IMP).replace(A_D, N_D).replace(A_SEN, N_SEN)
      ...
      .replace(A_STOP, N_STOP).replace(A_SAL, N_SAL).replace(A_G, N_G).replace(A_ANCHO2, N_ANCHO2).replace(A_ANCHO, N_ANCHO)
      .replace(A_FIN, N_FIN).replace("* 1.01", "* _SLIP"))
tp = bpip.replace(P_IMP, P_IMPN).replace(P_VIEJO, P_NUEVO)

try:
    assert N_G in tm, 'el parche del descuento NO se aplico'
    assert '_DESC' in tm and tm.count('_DESC') >= 2, 'falta _DESC en el motor'
    for txt, ruta in ((tm, MOT), (tp, PIP)):
        compile(txt, ruta, "exec")
    open(MOT, "w", encoding="utf-8").write(tm)
    open(PIP, "w", encoding="utf-8").write(tp)
    procs = []
    for nombre, cap, frac, tmin, pausa, stopd, mt, anc, reac, score, uco, uiv, usk, desde, ksup, stopop, tp, tmax, tpa, tabs, desc in V:
        env = dict(os.environ, RL="vodc", RL_CAP=cap, RL_SLIP="1.01",
                   ...
                   RL_UCO=uco, RL_UIV=uiv, RL_USK=usk, RL_DESDE=desde, RL_KSUP=ksup, RL_STOPOP=stopop, RL_TP=tp, RL_TMAX=tmax, RL_TPA=tpa, RL_TABS=tabs, RL_DESC=desc)
        ...
        print("lanzado %-12s desde %s descuento=%s" % (nombre, desde, desc), flush=True)
