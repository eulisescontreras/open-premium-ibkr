...
BMOT, BPIP = MOT + ".sz.bak", PIP + ".sz.bak"
...
# (nombre, capital, RL_SIZE, RL_FRAC, RL_TMIN, RL_FRENO, RL_FRENOF)
# Mejor hasta ahora: fracmin 18% con suelo 110 -> 600$ -> 46.761$ (77.9x), dd -31.2%, racha 7.
# Se ataca: (a) la FRACCION, (b) el SUELO, (c) la racha/drawdown con FRENO, (d) el capital minimo.
V = [
    ("s2_f18",  "600",  "fracmin", "0.18", "110", "0", "0.5"),   # referencia
    ("s2_f12",  "600",  "fracmin", "0.12", "110", "0", "0.5"),   # (a) fraccion
    ("s2_f15",  "600",  "fracmin", "0.15", "110", "0", "0.5"),
    ("s2_f22",  "600",  "fracmin", "0.22", "110", "0", "0.5"),
    ("s2_f30",  "600",  "fracmin", "0.30", "110", "0", "0.5"),
    ("s2_t090", "600",  "fracmin", "0.18", "90",  "0", "0.5"),   # (b) suelo
    ("s2_t140", "600",  "fracmin", "0.18", "140", "0", "0.5"),
    ("s2_t175", "600",  "fracmin", "0.18", "175", "0", "0.5"),
    ("s2_fr2",  "600",  "fracmin", "0.18", "110", "2", "0.5"),   # (c) freno racha>=2
    ("s2_fr3",  "600",  "fracmin", "0.18", "110", "3", "0.5"),
    ("s2_fr2h", "600",  "fracmin", "0.18", "110", "2", "0.33"),
    ("s2_c300", "300",  "fracmin", "0.18", "110", "0", "0.5"),   # (d) capital minimo viable
    ("s2_c400", "400",  "fracmin", "0.18", "110", "0", "0.5"),
    ("s2_c500", "500",  "fracmin", "0.18", "110", "0", "0.5"),
]
...
for txt, pat in ((bmot, A_IMP), (bmot, A_D), (bmot, A_SEN), (bmot, A_NQ), (bmot, A_NQ2),
                 ...
                 (bmot, A_FIN), (bpip, P_IMP), (bpip, P_VIEJO)):
...
tm = (bmot.replace(A_IMP, N_IMP).replace(A_D, N_D).replace(A_SEN, N_SEN)
      ...
      .replace(A_ANCHO2, N_ANCHO2).replace(A_ANCHO, N_ANCHO)
...
try:
    ...
    for nombre, cap, size, frac, tmin, freno, frenof in V:
        ...
        print("lanzado %-10s cap=%-5s frac=%s tmin=%s freno=%s" % (nombre, cap, frac, tmin, freno), flush=True)
