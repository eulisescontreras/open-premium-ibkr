...
_CMODO = _os.environ.get("RL_CMODO", "dobla")         # dobla | filtra | azar
_SEM = int(_os.environ.get("RL_SEM", "0"))            # semilla del control aleatorio
_FRAC_AZAR = float(_os.environ.get("RL_FRAC_AZAR", "0.31"))   # % de dias a doblar al azar
...
N_SEN = A_SEN + '''
        ...
        if _CMODO == "siempre":
            ...
            # CONTROL DECISIVO: doblar la MISMA proporcion de dias pero AL AZAR. Si esto da
            # lo mismo que la compresion, no hay señal: es apalancamiento disfrazado.
            _rnd.seed(_SEM * 100000 + int(fk.replace("-", "")))
            _dobla_hoy = _rnd.random() < _FRAC_AZAR
        if _COMPR:
            _pl = 0
...
N_NQ = ("'vert': True, 'nq': (min(nq * 2, _AC.TOPE_UNIDADES) if ((_CMODO in ('azar','siempre') and _dobla_hoy) "
        "or (_COMPR and _CMODO == 'dobla' and _plano.get((mm(h) // 3) * 3 - 3, 0) >= _COMPR)) "
        "else (nq if h >= C.DIABUENO_DESDE else 1))}")
...
assert "_dobla_hoy" in txt and txt.count("_CMODO") >= 3, "parche NO aplicado"
try:
    ...
    for nombre, compr, cmodo in V:
        env = dict(os.environ, RL_COMPR=compr, RL_CMODO=cmodo,
                   RL_SEM=str(procs.__len__() + 100), RL_FRAC_AZAR="0.31")
